...
class MenuScene extends Scene {
	...
	public function new (sm:SceneManager) {
		...
		// Agregar transicion en pantalla
		addChild(transitioner);
		hijos.push(transitioner);		
		
		// -------------------------- SIDESCROLLER -------------------------- //
		var sidescroll = new BackgroundSidescroller(800);
		...
		// Agregar sidescroller en pantalla
		addChild(sidescroll);
		...
		// -------------------------- VENTANA POPUP -------------------------- //
		var popup = new PopupWindow(this, 800, 600);
		
		var sprOpciones = showOptions();
		var sprAyuda = showHelp();
		var sprCreditos = showCredits();		
		
		// -------------------------- OPCIONES -------------------------- //
		var stick = new Bitmap ( Assets.getBitmapData("images/menu_stick.png") );
		stick.x = 676; 
		stick.y = 605 - stick.height;
		addChild(stick);
		
		var btn_opciones = new Button("images/menu_opciones.png", function() { popup.init(sprOpciones); }, 2);
		var btn_ayuda = new Button("images/menu_ayuda.png", function() { popup.init(sprAyuda); }, -2);
		var btn_creditos = new Button("images/menu_creditos.png", function() { popup.init(sprCreditos); }, 2);		
		...
		addChild(btn_opciones);
		addChild(btn_ayuda);
		addChild(btn_creditos);
		
		// -------------------------- MENU JUGAR -------------------------- //
		
		players = new Bitmap ( Assets.getBitmapData("images/menu_players.png") );
		addChild(players);
		players.x = 28;
		players.y = -players.height; //50
		
		// Botones (click area)
		link1p = new Bitmap ( Assets.getBitmapData("images/menu_players_boton.png") );
		link2p = new Bitmap ( Assets.getBitmapData("images/menu_players_boton.png") );
	...
	override public function init(){
		...
		Actuate.tween(players, 2, { y:0 } ).delay(0.5).onComplete(function() {
			link1p.x = link2p.x = players.x + 8;
			link1p.y = players.y + 197; 
			link2p.y = players.y + 340;
		});
	}
